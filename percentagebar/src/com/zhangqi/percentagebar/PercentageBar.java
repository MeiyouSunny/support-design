...
public class PercentageBar extends View {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (widthSpecMode == MeasureSpec.AT_MOST
			...
			setMeasuredDimension((int) max, startY + 10 + totalBarNum
					* (barWidth + 2 * 10));
			...
			setMeasuredDimension(widthSpecSize, startY + 10 + totalBarNum
					* (barWidth + 2 * 10));
		}
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		// 计算每条柱状图之间的间距
		deltaY = (measuredHeight - startY - barWidth * totalBarNum)
				/ totalBarNum;
		// 计算出每条竖线所代表的数值
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < totalBarNum; i++) {
			...
			canvas.drawText(respName.get(i), startX, startY + deltaY + i
					* (deltaY + barWidth) + 3 * barWidth / 4, mTextPaint);
			canvas.drawRect(startX + 7 * barWidth / 5, startY + deltaY + i
					* (deltaY + barWidth), currentBarProgress[i], startY
					+ deltaY + i * (deltaY + barWidth) + barWidth, mBarPaint);
		}
		/**
		 * 画竖线
		 */
		for (int i = 0; i < verticalLineNum; i++) {
			...
			canvas.drawLine((startX + 7 * barWidth / 5) + (i + 1) * deltaX,
					startY, (startX + 7 * barWidth / 5) + (i + 1) * deltaX,
					currentVerticalLineProgress, mLinePaint);
			canvas.drawText(numPerUnit * (i + 1) + unit,
					(startX + 7 * barWidth / 5) + (i + 1) * deltaX - barWidth,
					startY - barWidth / 5, mTextPaint);
		}
		/**
		 * 画最上面的横线
		 */
		...
		canvas.drawLine(stopX, startY, currentHorizentalLineProgress, startY,
				mLinePaint);
	}
